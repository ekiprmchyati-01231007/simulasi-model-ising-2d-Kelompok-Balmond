# Simulasi Model Ising 2D (Metode Monte Carlo)

Repository ini dibuat oleh **Kelompok Balmond** untuk memenuhi tugas mata kuliah **Fisika Komputasi (Case Base 2 : Transisi Fase - Metode Monte Carlo)**.

---

## 👥 Anggota Kelompok Balmond

1. **Eki Permata Cahya Hati** (NIM: 01231007)
2. **Melky Juli Kurniawan Silalahi** (NIM: 01231010)
3. **Yustiara Sampe Manggoali** (NIM: 01231019)

---

## 🔗 Hasil Pengerjaan Tugas

Hasil pengerjaan tugas dapat diakses melalui link berikut:
* **Halaman Laporan Utama:** [Klik di sini untuk melihat laporan](https://ekiprmchyati-01231007.github.io/simulasi-model-ising-2d-Kelompok-Balmond/)
